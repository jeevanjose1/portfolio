...
export default function ProcessStep({
  ...
  return (
    <div className="relative flex-1 flex flex-col items-center text-center px-4 transition-colors duration-300">
      ...
      <div className="relative z-10 w-20 h-20 rounded-lg bg-background border border-border flex items-center justify-center shadow-xl shadow-black/5 mb-8 group transition-all duration-500 hover:border-accent hover:shadow-accent/5">
        <Icon size={32} className="text-accent group-hover:scale-110 transition-transform" />
        {/* Number Badge — Bold uppercase pill */}
        <div className="absolute -top-3 -right-3 px-3 py-1 rounded-lg bg-accent text-white text-[9px] font-black uppercase tracking-widest border-2 border-background">
          Step {number}
        </div>
      </div>
      ...
      {/* Mobile Connector */}
      {!isLast && (
        <div className="md:hidden w-px h-16 bg-border my-8" />
      )}
    </div>
  );
}
