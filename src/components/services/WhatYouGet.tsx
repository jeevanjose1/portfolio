        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {items.map((item: { icon: string; title: string; description: string }, i: number) => {
            ...
            return (
              ...
              >
                <div
                  className="bg-section-alt rounded-lg p-8 border border-border hover:border-[color-mix(in_srgb,var(--color-accent)_30%,transparent)] hover:shadow-xl hover:shadow-[color-mix(in_srgb,var(--color-accent)_5%,transparent)] transition-all duration-300 group h-full flex flex-col"
                >
