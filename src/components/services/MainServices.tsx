...
function MainServiceCard({ service, index }: { service: MainServiceItem; index: number }) {
  ...
  return (
    <motion.div
      ...
      className="h-full"
    >
      <GradientCard className="h-full border border-gray-100 p-8 flex flex-col bg-white transition-all duration-500 hover:border-accent/30 hover:shadow-2xl hover:shadow-accent/5">
        <div className="w-14 h-14 rounded-lg bg-gray-50 border border-gray-100 flex items-center justify-center mb-6 group-hover:bg-accent group-hover:text-white transition-all duration-300">
          <Icon size={28} className="text-accent group-hover:text-white transition-colors" />
        </div>
        
        <h3 className="text-2xl font-heading font-black text-primary mb-3 group-hover:text-accent transition-colors duration-300 uppercase tracking-tight">
          {service.title}
        </h3>
        <p className="text-gray-500 leading-relaxed mb-8 flex-grow font-body italic text-sm">
        ...
        <ul className="space-y-3 mb-8">
          {service.features.map((feature, i) => (
            <li key={i} className="flex items-start gap-3">
              ...
              <span className="text-xs font-black uppercase tracking-widest text-gray-700">{feature}</span>
            </li>
          ))}
        </ul>

        <div className="pt-6 border-t border-gray-50 mt-auto flex items-center justify-between relative z-10">
          <span className="text-sm font-black uppercase tracking-widest text-primary">Starts at {service.startingPrice}</span>
          <Link
            href={`/services/${service.slug}`}
            className="group/btn flex items-center gap-2 text-sm font-black uppercase tracking-widest text-accent hover:text-primary transition-colors duration-300"
          >
            Learn More
