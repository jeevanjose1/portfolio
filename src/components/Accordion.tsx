...
export default function Accordion({ question, answer }: AccordionProps) {
  ...
  return (
    <div className={`rounded-lg bg-background border mb-4 last:mb-0 transition-all duration-500 overflow-hidden ${isOpen ? 'border-accent shadow-2xl shadow-accent/5' : 'border-border hover:border-accent/20'}`}>
      <button
      ...
      >
        ...
        <div
          className={`flex-shrink-0 ml-6 w-10 h-10 rounded-full flex items-center justify-center transition-all duration-300 ${isOpen ? 'bg-accent text-white rotate-180' : 'bg-section-alt text-muted-foreground group-hover:bg-accent/10 group-hover:text-accent'}`}
        >
